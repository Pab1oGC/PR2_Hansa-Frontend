...
interface FileCardProps {
  ...
  const FileCard: React.FC<FileCardProps> = ({ file }) => {
    return (
      <div
        className={`bg-gradient-to-b ${getImportanceColor(
          file.importance
        )} bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow border border-gray-100`}
        role="article"
        tabIndex={0}
      >
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-2">
            {file.fileType === "documento" ? (
              ...
              <FiFolder className="w-5 h-5 text-amber-600" />
            )}
            <h3 className="font-medium text-gray-900">{file.name}</h3>
          </div>
          <div className="flex items-center space-x-0.5">
            {[...Array(file.importance)].map((_, i) => (
